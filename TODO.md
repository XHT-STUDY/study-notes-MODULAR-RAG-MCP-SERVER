...
- [!] 0.1.2 仓库地图：`core`、`ingestion`、`libs`、`mcp_server`、`observability` 的职责和协作关系。（A 已通过；B、C 待复查）
  - [x] 0.1.2-A 五个主模块各自负责什么、不负责什么、代表入口在哪里。
  - [!] 0.1.2-B 五个模块的依赖方向，以及为什么需要分层而不是放进一个 Pipeline。（理解内容已讲解，检查题主动延后）
  - [!] 0.1.2-C 给定新需求或故障，能判断应该定位/修改哪个模块并完成讲师验收。（主动跳过）
- [!] 0.1.3 工程事实基线：技术栈、代码/测试规模、分支状态、文档与代码漂移。（知识内容已讲解，检查题主动延后）
...
### 2026-08-18｜暂存 0.1.2-B 检查题，推进到 0.1.2-C

- 按学习者选择，0.1.2-B 的理解、追问和讲师验收暂不进行，标记为 `[!]`，最终总复习前补齐。
- 已讲内容保留：代码依赖与运行时数据流的区别、理想分层、当前跨层耦合，以及替换 Provider 时的影响面。
- 当前起点切换为 0.1.2-C：通过新需求和故障场景，练习从现象定位责任模块、调用链和可能的修复边界。

### 2026-08-18｜跳过 0.1.2-C，进入 0.1.3 工程事实基线

- 按学习者选择，0.1.2-C 整节主动跳过并标记为 `[!]`；0.1.2 当前整体状态为待复查，A 已通过，B、C 留待总复习补齐。
- 当前仓库事实：`src/` 有 126 个 Python 文件，`tests/` 有 108 个 Python 文件；测试目录中 unit 83、integration 13、e2e 5 个 Python 文件，其余为公共配置与 fixtures。
- 当前分支为 `main`，跟踪 `origin/main`；工作区当前只显示伴读进度文件 `TODO.md` 被修改，同时存在若干 pytest 临时目录权限警告。
- 已确认一个文档漂移样例：`ARCHITECTURE.md` 仍描述和注册“三个 MCP Tool”，而当前 README 与 `protocol_handler.py` 已包含第四个 `agent_query`。
- 配置事实：项目运行要求 Python `>=3.12,<3.13`，但 Ruff 和 mypy 的目标版本仍为 Python 3.10；该差异保留到工程演进章节分析。

### 2026-08-18｜暂存 0.1.3 检查题，进入 0.2 运行基线

- 按学习者选择，0.1.3 的理解检查与讲师验收主动延后，标记为 `[!]`；已核对的工程事实和文档漂移证据继续保留。
- 当前开始 0.2：阅读并运行 `scripts/self_check.py`，区分环境可用性检查、核心行为测试和完整业务正确性验证。

### 2026-08-18｜0.2 运行基线：环境自检通过

- 使用项目虚拟环境 Python 3.12.4 执行 `scripts/self_check.py --json`，结果为 9 项 OK、0 项 FAIL，进程退出码为 0。
- 阻塞检查通过：Python 版本、配置加载、关键依赖导入、数据目录写入、Chroma 连接与探针集合、SQLite 建表、BM25 JSON 往返写入。
- 非阻塞检查也通过：Trace 日志可追加、LLM 与 Embedding API Key 已配置。
- 注意：自检会创建目录/SQLite schema、短暂创建后删除 Chroma 探针集合，并向 `logs/traces.jsonl` 追加一条记录；它证明环境具备启动条件，不证明完整摄取、检索、生成和回答质量都正确。
- 当前停点：理解九项检查的保护范围、阻塞与非阻塞状态，以及自检与 Unit/Integration/E2E 测试的边界。
